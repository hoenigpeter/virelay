...
def main(variant, attribution_file, analysis_file, class_indices, label_map_file, n_eigval, n_clusters, n_neighbors):
    ...
    for class_index in class_indices:
        ...
        json_object = json.dumps(eigenvalues_list, indent = 4)
        with open('./metrics/eigenvalues.json', "w") as outfile:
            outfile.write(json_object)

        ## TSNE
        #
        print("TSNE Shape: ", tsne.shape)
        tsne_list = tsne.tolist()
        json_object = json.dumps(tsne_list, indent = 4)
        with open('./metrics/tsne_' + str(class_index) + '.json', "w") as outfile:
            outfile.write(json_object)

        ## KMEANS
        print("kmeans Shape: ", np.array(kmeans).shape)
        kmeans_list = np.array(kmeans).tolist()
        json_object = json.dumps(kmeans_list, indent = 4)
        with open('./metrics/kmeans_' + str(class_index) + '.json', "w") as outfile:
            outfile.write(json_object)

        ## Embedding
        print("Embeddings Shape: ", embedding.shape)
        embedding_list = np.array(embedding).tolist()
        json_object = json.dumps(embedding_list, indent = 4)
        with open('./metrics/embedding_' + str(class_index) + '.json', "w") as outfile:
            outfile.write(json_object)
