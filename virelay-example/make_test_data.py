...
@click.option('--num-classes', default=1)
@click.option('--num-samples', default=153)
...
def main(
    ...
    for label_s in range(num_classes):
        data = np.random.uniform(0, 1, size=(num_samples, 3, 224, 224))
        label = np.array([label_s] * num_samples)
        print("Label: ", label.shape)
        print("data: ", data.shape)

        attrib = np.random.uniform(-1, 1, size=(num_samples, 3, 224, 224))
        print("attrib: ", attrib.shape)
